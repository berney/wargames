...
char *argv[];
{
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, BOOTPARAMPROG, BOOTPARAMVERS, bootparamprog_1, proto)) {
			_msgout("unable to register (BOOTPARAMPROG, BOOTPARAMVERS, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, BOOTPARAMPROG, BOOTPARAMVERS, bootparamprog_1, proto)) {
			_msgout("unable to register (BOOTPARAMPROG, BOOTPARAMVERS, tcp).");
			exit(1);
		}
	}
